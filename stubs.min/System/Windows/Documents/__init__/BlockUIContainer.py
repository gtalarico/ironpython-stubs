class BlockUIContainer(Block, IInputElement, IAnimatable, IFrameworkInputElement, ISupportInitialize, IQueryAmbient, IAddChild):
    """
    A block-level flow content element which enables System.Windows.UIElement elements (i.e. a System.Windows.Controls.Button) to be embedded (hosted) in flow content.
    
    BlockUIContainer()
    BlockUIContainer(uiElement: UIElement)
    """
    ...
    def __new__(self, uiElement=None):
        """
        __new__(cls: type)
        __new__(cls: type, uiElement: UIElement)
        """
        pass

    Child = property(lambda self: object(), lambda self, v: None, lambda self: None)  # default
    """Gets or sets the System.Windows.UIElement hosted by the System.Windows.Documents.BlockUIContainer.

Get: Child(self: BlockUIContainer) -> UIElement

Set: Child(self: BlockUIContainer) = value
"""
