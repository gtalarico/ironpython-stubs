class GC(object):
 ...
 def Collect(generation=None,mode=None,blocking=None,compacting=None):
  ...
  Collect(generation: int,mode: GCCollectionMode,blocking: bool)Collect(generation: int,mode: GCCollectionMode,blocking: bool,compacting: bool)Collect(generation: int,mode: GCCollectionMode)
   Forces a garbage collection from generation zero through a specified 
    generation,at a time specified by a System.GCCollectionMode value.
 ...
 def CollectionCount(generation):
  ...
  CollectionCount(generation: int) -> int
   ...
   generation: The generation of objects for which the garbage collection count is to be 
    determined.
 ...
 def GetGeneration(*__args):
  ...
  GetGeneration(wo: WeakReference) -> int
   ...
   Returns the current generation number of the target of a specified weak 
    reference.
  
  
   wo: A System.WeakReference that refers to the target object whose generation number 
    is to be determined.
 ...
 def GetTotalMemory(forceFullCollection):
  ...
  GetTotalMemory(forceFullCollection: bool) -> Int64
   ...
   forceFullCollection: true to indicate that this method can wait for garbage collection to occur 
    before returning; otherwise,false.
  
   Returns: A number that is the best available approximation of the number of bytes 
 ...
 def RegisterForFullGCNotification(maxGenerationThreshold,largeObjectHeapThreshold):
  ...
  RegisterForFullGCNotification(maxGenerationThreshold: int,largeObjectHeapThreshold: int)
   ...
   maxGenerationThreshold: A number between 1 and 99 that specifies when the notification should be raised 
    based on the objects surviving in generation 2.
  
   largeObjectHeapThreshold: A number between 1 and 99 that specifies when the notification should be raised 
    based on objects allocated in the large object heap.
  """
  pass
 @staticmethod
 ...
 def WaitForFullGCApproach(millisecondsTimeout=None):
  ...
  WaitForFullGCApproach(millisecondsTimeout: int) -> GCNotificationStatus
   ...
   Returns,in a specified time-out period,the status of a registered 
    notification for determining whether a full garbage collection by the common 
    language runtime is imminent.
  
  
   millisecondsTimeout: The length of time to wait before a notification status can be obtained. 
  ...
  WaitForFullGCApproach() -> GCNotificationStatus
   ...
   Returns the status of a registered notification for determining whether a full 
    garbage collection by the common langauge runtime is imminent.
 ...
 def WaitForFullGCComplete(millisecondsTimeout=None):
  ...
  WaitForFullGCComplete(millisecondsTimeout: int) -> GCNotificationStatus
   ...
   Returns,in a specified time-out period,the status of a registered 
    notification for determining whether a full garbage collection by common 
