class StreamGeometryContext(DispatcherObject,IDisposable):
 ...
 def ArcTo(self,point,size,rotationAngle,isLargeArc,sweepDirection,isStroked,isSmoothJoin):
  ...
  ArcTo(self: StreamGeometryContext,point: Point,size: Size,rotationAngle: float,isLargeArc: bool,sweepDirection: SweepDirection,isStroked: bool,isSmoothJoin: bool)
   ...
   size: The radii (half the width and half the height) of an oval whose perimeter is 
    used to draw the angle. If the oval is very rounded in all directions,the arc 
    will be rounded,if it is nearly flat,so will the arc. For example,a very 
    large width and height would represent a very large oval,which would give a 
    slight curvature for the angle.
  
   rotationAngle: The rotation angle of the oval that specifies the curve. The curvature of the 
    arc can be rotated with this parameter.
  
   isLargeArc: true to draw the arc greater than 180 degrees; otherwise,false.
   sweepDirection: A value that indicates whether the arc is drawn in the 
    System.Windows.Media.SweepDirection.Clockwise or 
    System.Windows.Media.SweepDirection.Counterclockwise direction.
  
   isStroked: true to make the segment stroked when a System.Windows.Media.Pen is used to 
    render the segment; otherwise,false.
  
   isSmoothJoin: true to treat the join between this segment and the previous segment as a 
    corner when stroked with a System.Windows.Media.Pen; otherwise,false.
  """
  pass
 def BeginFigure(self,startPoint,isFilled,isClosed):
  ...
  BeginFigure(self: StreamGeometryContext,startPoint: Point,isFilled: bool,isClosed: bool)
   ...
   isFilled: true to use the area contained by this figure for hit-testing,rendering,and 
    clipping; otherwise,false.
  
   isClosed: true to close the figure; otherwise,false. For example,if two connecting 
    lines are drawn,and isClosed is set to false,the drawing will just be of two 
    lines but if isClosed is set to true,the two lines will be closed to create a 
    triangle.
  """
  pass
