class AnimationClock(Clock):
    ...
    def GetCurrentValue(self, defaultOriginValue, defaultDestinationValue):
        ...
        GetCurrentValue(self: AnimationClock, defaultOriginValue: object, defaultDestinationValue: object) -> object
            ...
            defaultOriginValue: The origin value provided to the clock if its animation does not have its own start value. If this clock is the first in a composition chain it will be the base value of the property being animated; 
             otherwise it will be the value returned by the previous clock in the chain
        
            defaultDestinationValue: The destination value provided to the clock if its animation does not have its own destination value. If this clock is the first in a composition chain it will be the base value of the property being 
