class HtmlWindow(object):
 ...
 def Confirm(self,message):
  ...
  Confirm(self: HtmlWindow,message: str) -> bool
   ...
   Returns: true if the user clicked Yes; false if the user clicked No or closed the dialog 
    box.
  """
  pass
 ...
 def Navigate(self,*__args):
  ...
  Navigate(self: HtmlWindow,url: Uri)
   ...
   url: The location,specified as a System.Uri,of the document or object to display 
 ...
 def Open(self,*__args):
  ...
  Open(self: HtmlWindow,url: Uri,target: str,windowOptions: str,replaceEntry: bool) -> HtmlWindow
   ...
   target: The name of the window in which to open the resource. This can be a 
    developer-supplied name,or one of the following special values:_blank: Opens 
    url in a new window. Works the same as a call to 
    System.Windows.Forms.HtmlWindow.OpenNew(System.String,System.String)._media: 
    Opens url in the Media bar. _parent: Opens url in the window that created the 
    current window._search: Opens url in the Search bar._self: Opens url in the 
    current window. _top: If called against a window belonging to a FRAME element,
    opens url in the window hosting its FRAMESET. Otherwise,acts the same as 
    _self.
  
   windowOptions: A comma-delimited string consisting of zero or more of the following options in 
    the form name=value. Except for the left,top,height,and width options,which 
    take arbitrary integers,each option accepts yes or 1,and no or 0,as valid 
    values.channelmode: Used with the deprecated channels technology of Internet 
    Explorer 4.0. Default is no.directories: Whether the window should display 
    directory navigation buttons. Default is yes. height: The height of the 
    window's client area,in pixels. The minimum is 100; attempts to open a window 
    smaller than this will cause the window to open according to The Internet 
    Explorer defaults. left: The left (x-coordinate) position of the window,
    relative to the upper-left corner of the user's screen,in pixels. Must be a 
    positive integer.location: Whether to display the Address bar,which enables 
    users to navigate the window to a new URL. Default is yes. menubar: Whether to 
    display menus on the new window. Default is yes.resizable: Whether the window 
    can be resized by the user. Default is yes.scrollbars: Whether the window has 
    horizontal and vertical scroll bars. Default is yes.status: Whether the window 
    has a status bar at the bottom. Default is yes.titlebar: Whether the title of 
    the current page is displayed. Setting this option to no has no effect within a 
    managed application; the title bar will always appear.toolbar: Whether toolbar 
    buttons such as Back,Forward,and Stop are visible. Default is yes.top: The 
    top (y-coordinate) position of the window,relative to the upper-left corner of 
    the user's screen,in pixels. Must be a positive integer.width: The width of 
    the window's client area,in pixels. The minimum is 100; attempts to open a 
    window smaller than this will cause the window to open according to The 
    Internet Explorer defaults.
  
   replaceEntry: Whether url replaces the current window's URL in the navigation history. This 
    will effect the operation of methods on the System.Windows.Forms.HtmlHistory 
    class.
  
   Returns: An System.Windows.Forms.HtmlWindow representing the new window,or the 
    previously created window named by the target parameter.
  ...
  Open(self: HtmlWindow,urlString: str,target: str,windowOptions: str,replaceEntry: bool) -> HtmlWindow
   ...
   target: The name of the window in which to open the resource. This may be a 
    developer-supplied name,or one of the following special values:_blank: Opens 
    url in a new window. Works the same as a call to 
    System.Windows.Forms.HtmlWindow.OpenNew(System.String,System.String)._media: 
    Opens url in the Media bar. _parent: Opens url in the window that created the 
 ...
 def OpenNew(self,*__args):
  ...
  OpenNew(self: HtmlWindow,url: Uri,windowOptions: str) -> HtmlWindow
   ...
   url: The Uniform Resource Locator that describes the location of the file to load.
   windowOptions: A comma-delimited string consisting of zero or more of the following options in 
    the form name=value. See 
    System.Windows.Forms.HtmlWindow.Open(System.String,System.String,System.String,S
    ystem.Boolean) for a full description of the valid options.
 ...
 def ResizeTo(self,*__args):
  ...
  ResizeTo(self: HtmlWindow,size: Size)
   ...
   size: A System.Drawing.Size describing the desired width and height of the window,in 
    pixels. Must be 100 pixels or more in both dimensions.
  
  ResizeTo(self: HtmlWindow,width: int,height: int)
   Changes the size of the window to the specified dimensions.
  
   width: Describes the desired width of the window,in pixels. Must be 100 pixels or 
    more.
  
   height: Describes the desired height of the window,in pixels. Must be 100 pixels or 
    more.
  """
  pass
 def ScrollTo(self,*__args):
  ...
  ScrollTo(self: HtmlWindow,point: Point)
   ...
   point: The x- and y-coordinates,relative to the top-left corner of the current 
    window,toward which the page should scroll.
  
  ScrollTo(self: HtmlWindow,x: int,y: int)
   Scrolls the window to the designated position.
  
   x: The x-coordinate,relative to the top-left corner of the current window,toward 
    which the page should scroll.
  
   y: The y-coordinate,relative to the top-left corner of the current window,toward 
    which the page should scroll.
  """
  pass
