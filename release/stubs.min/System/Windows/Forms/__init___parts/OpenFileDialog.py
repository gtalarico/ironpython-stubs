class OpenFileDialog(FileDialog,IComponent,IDisposable):
 ...
 def HookProc(self,*args):
  ...
  HookProc(self: FileDialog,hWnd: IntPtr,msg: int,wparam: IntPtr,lparam: IntPtr) -> IntPtr
   ...
   Returns: Returns zero if the default dialog box procedure processes the message; returns 
    a nonzero value if the default dialog box procedure ignores the message.
  """
  pass
 ...
 def OpenFile(self):
  ...
  OpenFile(self: OpenFileDialog) -> Stream
   ...
   Opens the file selected by the user,with read-only permission. The file is 
    specified by the System.Windows.Forms.FileDialog.FileName property.
 ...
 def RunDialog(self,*args):
  ...
  RunDialog(self: FileDialog,hWndOwner: IntPtr) -> bool
   ...
   hWndOwner: A value that represents the window handle of the owner window for the common 
    dialog box.
