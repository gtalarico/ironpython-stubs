# encoding: utf-8
# module Wms.RemotingImplementation.EdiMessaging.Handlers calls itself Handlers
# from Wms.RemotingImplementation,Version=1.0.0.0,Culture=neutral,PublicKeyToken=null
# by generator 1.145
# no doc
# no imports

# no functions
# classes

class CheckLpIntegrity_StockOnLocationChangedMessageHandler:
 """ CheckLpIntegrity_StockOnLocationChangedMessageHandler() """
 def CanHandle(self,message):
  """ CanHandle(self: CheckLpIntegrity_StockOnLocationChangedMessageHandler,message: IMessage) -> bool """
  pass
 def Handle(self,message):
  """ Handle(self: CheckLpIntegrity_StockOnLocationChangedMessageHandler,message: IMessage) -> HandleResult """
  pass
 def __init__(self,*args):
  """ x.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signature """
  pass

class ProcessDirectOrderMessageHandler:
 """ ProcessDirectOrderMessageHandler(messaging: IMessaging,directOrderLogger: DirectOrderLogger) """
 def CanHandle(self,message):
  """ CanHandle(self: ProcessDirectOrderMessageHandler,message: IMessage) -> bool """
  pass
 def Handle(self,message):
  """ Handle(self: ProcessDirectOrderMessageHandler,message: IMessage) -> HandleResult """
  pass
 def __init__(self,*args):
  """ x.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signature """
  pass
 @staticmethod
 def __new__(self,messaging,directOrderLogger):
  """ __new__(cls: type,messaging: IMessaging,directOrderLogger: DirectOrderLogger) """
  pass

class ProcessPreReceiptMessageHandler:
 """ ProcessPreReceiptMessageHandler() """
 def CanHandle(self,message):
  """ CanHandle(self: ProcessPreReceiptMessageHandler,message: IMessage) -> bool """
  pass
 def Handle(self,message):
  """ Handle(self: ProcessPreReceiptMessageHandler,message: IMessage) -> HandleResult """
  pass
 def __init__(self,*args):
  """ x.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signature """
  pass

class ProcessSalesOrderMessageHandler:
 """ ProcessSalesOrderMessageHandler(outbound: Outbound,stockManager: IStockManager) """
 def CanHandle(self,message):
  """ CanHandle(self: ProcessSalesOrderMessageHandler,message: IMessage) -> bool """
  pass
 def Handle(self,message):
  """ Handle(self: ProcessSalesOrderMessageHandler,message: IMessage) -> HandleResult """
  pass
 def __init__(self,*args):
  """ x.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signature """
  pass
 @staticmethod
 def __new__(self,outbound,stockManager):
  """ __new__(cls: type,outbound: Outbound,stockManager: IStockManager) """
  pass

# variables with complex values

