...
class IAdviseSink:
 ...
 def OnDataChange(self,format,stgmedium):
  ...
  OnDataChange(self: IAdviseSink,format: FORMATETC,stgmedium: STGMEDIUM) -> (FORMATETC,STGMEDIUM)
   ...
   stgmedium: A System.Runtime.InteropServices.ComTypes.STGMEDIUM,passed by reference,which defines the storage medium (global memory,disk file,storage object,stream object,Graphics Device Interface (GDI) object,
    or undefined) and ownership of that medium for the calling data object.
  """
  pass
...
class IDataObject:
 ...
 def DAdvise(self,pFormatetc,advf,adviseSink,connection):
  ...
  DAdvise(self: IDataObject,pFormatetc: FORMATETC,advf: ADVF,adviseSink: IAdviseSink) -> (int,FORMATETC,int)
   ...
   Creates a connection between a data object and an advisory sink. This method is called by an object that supports an advisory sink and enables the advisory sink to be notified of changes in the object's 
    data.
   ...
   Returns: This method supports the standard return values E_INVALIDARG,E_UNEXPECTED,and E_OUTOFMEMORY,as well as the following: ValueDescriptionS_OKThe advisory connection was created.E_NOTIMPLThis method is not 
    implemented on the data object.DV_E_LINDEXThere is an invalid value for System.Runtime.InteropServices.ComTypes.FORMATETC.lindex; currently,only -1 is supported.DV_E_FORMATETCThere is an invalid value for 
    the pFormatetc parameter.OLE_E_ADVISENOTSUPPORTEDThe data object does not support change notification.
  """
  pass
 def DUnadvise(self,connection):
  ...
  DUnadvise(self: IDataObject,connection: int)
   ...
   connection: A DWORD token that specifies the connection to remove. Use the value returned by 
    System.Runtime.InteropServices.ComTypes.IDataObject.DAdvise(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.ADVF,System.Runtime.InteropServices.ComTypes.IAdviseSin
    k,System.Int32@) when the connection was originally established.
  """
  pass
 def EnumDAdvise(self,enumAdvise):
  ...
  EnumDAdvise(self: IDataObject) -> (int,IEnumSTATDATA)
   ...
   Returns: This method supports the standard return value E_OUTOFMEMORY,as well as the following:ValueDescriptionS_OKThe enumerator object is successfully instantiated or there are no 
 ...
 def EnumFormatEtc(self,direction):
  ...
  EnumFormatEtc(self: IDataObject,direction: DATADIR) -> IEnumFORMATETC
   ...
   Returns: This method supports the standard return values E_INVALIDARG and E_OUTOFMEMORY,as well as the following:ValueDescriptionS_OKThe enumerator object was successfully created.E_NOTIMPLThe direction specified 
    by the direction parameter is not supported.OLE_S_USEREGRequests that OLE enumerate the formats from the registry.
  """
  pass
 def GetCanonicalFormatEtc(self,formatIn,formatOut):
  ...
  GetCanonicalFormatEtc(self: IDataObject,formatIn: FORMATETC) -> (int,FORMATETC,FORMATETC)
   ...
   Provides a standard System.Runtime.InteropServices.ComTypes.FORMATETC structure that is logically equivalent to a more complex structure. Use this method to determine whether two different 
    System.Runtime.InteropServices.ComTypes.FORMATETC structures would return the same data,removing the need for duplicate rendering.
  
  
   formatIn: A pointer to a System.Runtime.InteropServices.ComTypes.FORMATETC structure,passed by reference,that defines the format,medium,and target device that the caller would like to use to retrieve data in a 
    subsequent call such as System.Runtime.InteropServices.ComTypes.IDataObject.GetData(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.STGMEDIUM@). The 
    System.Runtime.InteropServices.ComTypes.TYMED member is not significant in this case and should be ignored.
  
   Returns: This method supports the standard return values E_INVALIDARG,E_UNEXPECTED,and E_OUTOFMEMORY,as well as the following: ValueDescriptionS_OKThe returned System.Runtime.InteropServices.ComTypes.FORMATETC 
    structure is different from the one that was passed.DATA_S_SAMEFORMATETCThe System.Runtime.InteropServices.ComTypes.FORMATETC structures are the same and null is returned in the formatOut 
    parameter.DV_E_LINDEXThere is an invalid value for System.Runtime.InteropServices.ComTypes.FORMATETC.lindex; currently,only -1 is supported.DV_E_FORMATETCThere is an invalid value for the pFormatetc 
 ...
 def GetData(self,format,medium):
  ...
  GetData(self: IDataObject,format: FORMATETC) -> (FORMATETC,STGMEDIUM)
   ...
   Obtains data from a source data object. The 
    System.Runtime.InteropServices.ComTypes.IDataObject.GetData(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.STGMEDIUM@) method,which is called by a data 
    consumer,renders the data described in the specified System.Runtime.InteropServices.ComTypes.FORMATETC structure and transfers it through the specified System.Runtime.InteropServices.ComTypes.STGMEDIUM 
   ...
   format: A pointer to a System.Runtime.InteropServices.ComTypes.FORMATETC structure,passed by reference,that defines the format,medium,and target device to use when passing the data. It is possible to specify 
    more than one medium by using the Boolean OR operator,allowing the method to choose the best medium among those specified.
  """
  pass
 def GetDataHere(self,format,medium):
  ...
  GetDataHere(self: IDataObject,format: FORMATETC,medium: STGMEDIUM) -> (FORMATETC,STGMEDIUM)
   ...
   Obtains data from a source data object. This method,which is called by a data consumer,differs from the 
    System.Runtime.InteropServices.ComTypes.IDataObject.GetData(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.STGMEDIUM@) method in that the caller must allocate 
    and free the specified storage medium.
  
  
   format: A pointer to a System.Runtime.InteropServices.ComTypes.FORMATETC structure,passed by reference,that defines the format,medium,and target device to use when passing the data. Only one medium can be 
    specified in System.Runtime.InteropServices.ComTypes.TYMED,and only the following System.Runtime.InteropServices.ComTypes.TYMED values are valid: 
    System.Runtime.InteropServices.ComTypes.TYMED.TYMED_ISTORAGE,System.Runtime.InteropServices.ComTypes.TYMED.TYMED_ISTREAM,System.Runtime.InteropServices.ComTypes.TYMED.TYMED_HGLOBAL,or 
    System.Runtime.InteropServices.ComTypes.TYMED.TYMED_FILE.
  
   medium: A System.Runtime.InteropServices.ComTypes.STGMEDIUM,passed by reference,that defines the storage medium containing the data being transferred. The medium must be allocated by the caller and filled in by 
    System.Runtime.InteropServices.ComTypes.IDataObject.GetDataHere(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.STGMEDIUM@). The caller must also free the medium. 
    The implementation of this method must always supply a value of null for the System.Runtime.InteropServices.ComTypes.STGMEDIUM.pUnkForRelease member of the System.Runtime.InteropServices.ComTypes.STGMEDIUM 
    structure that this parameter points to.
  """
  pass
 def QueryGetData(self,format):
  ...
  QueryGetData(self: IDataObject,format: FORMATETC) -> (int,FORMATETC)
   ...
   Determines whether the data object is capable of rendering the data described in the System.Runtime.InteropServices.ComTypes.FORMATETC structure. Objects attempting a paste or drop operation can call this 
    method before calling System.Runtime.InteropServices.ComTypes.IDataObject.GetData(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.STGMEDIUM@) to get an indication 
   ...
   Returns: This method supports the standard return values E_INVALIDARG,E_UNEXPECTED,and E_OUTOFMEMORY,as well as the following: ValueDescriptionS_OKA subsequent call to 
    System.Runtime.InteropServices.ComTypes.IDataObject.GetData(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.STGMEDIUM@) would probably be successful.DV_E_LINDEXAn 
    invalid value for System.Runtime.InteropServices.ComTypes.FORMATETC.lindex; currently,only -1 is supported.DV_E_FORMATETCAn invalid value for the pFormatetc parameter.DV_E_TYMEDAn invalid 
    System.Runtime.InteropServices.ComTypes.FORMATETC.tymed value.DV_E_DVASPECTAn invalid System.Runtime.InteropServices.ComTypes.FORMATETC.dwAspect value.OLE_E_NOTRUNNINGThe application is not running.
  """
  pass
 def SetData(self,formatIn,medium,release):
  ...
  SetData(self: IDataObject,formatIn: FORMATETC,medium: STGMEDIUM,release: bool) -> (FORMATETC,STGMEDIUM)
   ...
   release: true to specify that the data object called,which implements 
    System.Runtime.InteropServices.ComTypes.IDataObject.SetData(System.Runtime.InteropServices.ComTypes.FORMATETC@,System.Runtime.InteropServices.ComTypes.STGMEDIUM@,System.Boolean),owns the storage medium 
    after the call returns. This means that the data object must free the medium after it has been used by calling the ReleaseStgMedium function. false to specify that the caller retains ownership of the 
    storage medium,and the data object called uses the storage medium for the duration of the call only.
  """
  pass
 def __init__(self,*args):
  """ x.__init__(...) initializes x; see x.__class__.__doc__ for signaturex.__init__(...) initializes x; see x.__class__.__doc__ for signature """
...
class IMoniker:
 ...
 def CommonPrefixWith(self,pmkOther,ppmkPrefix):
  ...
  CommonPrefixWith(self: IMoniker,pmkOther: IMoniker) -> IMoniker
   ...
   pmkOther: A reference to the IMoniker interface on another moniker to compare with the current moniker for a common prefix.
  """
  pass
 def ComposeWith(self,pmkRight,fOnlyIfNotGeneric,ppmkComposite):
  ...
  ComposeWith(self: IMoniker,pmkRight: IMoniker,fOnlyIfNotGeneric: bool) -> IMoniker
   ...
   fOnlyIfNotGeneric: true to indicate that the caller requires a nongeneric composition. The operation proceeds only if pmkRight is a moniker class that the current moniker can combine with in some way other than forming a 
    generic composite. false to indicate that the method can create a generic composite if necessary.
  """
  pass
 ...
 def IsRunning(self,pbc,pmkToLeft,pmkNewlyRunning):
  ...
  IsRunning(self: IMoniker,pbc: IBindCtx,pmkToLeft: IMoniker,pmkNewlyRunning: IMoniker) -> int
   ...
   Returns: An S_OKHRESULT value if the moniker is running; an S_FALSEHRESULT value if the moniker is not running; or an E_UNEXPECTEDHRESULT value.
  """
  pass
 ...
 def ParseDisplayName(self,pbc,pmkToLeft,pszDisplayName,pchEaten,ppmkOut):
  ...
  ParseDisplayName(self: IMoniker,pbc: IBindCtx,pmkToLeft: IMoniker,pszDisplayName: str) -> (int,IMoniker)
   ...
   Reads as many characters of the specified display name as the 
    System.Runtime.InteropServices.ComTypes.IMoniker.ParseDisplayName(System.Runtime.InteropServices.ComTypes.IBindCtx,System.Runtime.InteropServices.ComTypes.IMoniker,System.String,System.Int32@,System.Runtime
    .InteropServices.ComTypes.IMoniker@) understands and builds a moniker corresponding to the portion read.
...
class IStream:
 ...
 def Revert(self):
  """
  Revert(self: IStream)
   Discards all changes that have been made to a transacted stream since the last System.Runtime.InteropServices.ComTypes.IStream.Commit(System.Int32) call.
  """
  pass
 ...
 def Stat(self,pstatstg,grfStatFlag):
  ...
  Stat(self: IStream,grfStatFlag: int) -> STATSTG
   ...
   grfStatFlag: Members in the STATSTG structure that this method does not return,thus saving some memory allocation operations.
  """
  pass
 ...
 def Write(self,pv,cb,pcbWritten):
  ...
  Write(self: IStream,pv: Array[Byte],cb: int,pcbWritten: IntPtr)
   ...
   pcbWritten: On successful return,contains the actual number of bytes written to the stream object. If the caller sets this pointer to System.IntPtr.Zero,this method does not provide the actual number of bytes 
    written.
  """
  pass
 def __init__(self,*args):
...
class ITypeInfo:
 ...
 def GetDllEntry(self,memid,invKind,pBstrDllName,pBstrName,pwOrdinal):
  ...
  GetDllEntry(self: ITypeInfo,memid: int,invKind: INVOKEKIND,pBstrDllName: IntPtr,pBstrName: IntPtr,pwOrdinal: IntPtr)
   ...
   pwOrdinal: If not null,and the function is defined by an ordinal,then lpwOrdinal is set to point to the ordinal.
  """
  pass
 ...
 def ReleaseVarDesc(self,pVarDesc):
  ...
  ReleaseVarDesc(self: ITypeInfo,pVarDesc: IntPtr)
   Releases a VARDESC structure previously returned by the System.Runtime.InteropServices.ComTypes.ITypeInfo.GetVarDesc(System.Int32,System.IntPtr@) method.
...
class ITypeLib2(ITypeLib):
 ...
 def FindName(self,szNameBuf,lHashVal,ppTInfo,rgMemId,pcFound):
  ...
  FindName(self: ITypeLib2,szNameBuf: str,lHashVal: int,pcFound: Int16) -> (Array[ITypeInfo],Array[int],Int16)
   ...
   lHashVal: A hash value to speed up the search,computed by the LHashValOfNameSys function. If lHashVal is 0,a value is computed.
   pcFound: On entry,a value,passed by reference,that indicates how many instances to look for. For example,pcFound=1 can be called to find the first occurrence. The search stops when one instance is found.On 
 ...
 def GetTypeInfoOfGuid(self,guid,ppTInfo):
  ...
  GetTypeInfoOfGuid(self: ITypeLib2,guid: Guid) -> (Guid,ITypeInfo)
   ...
   guid: The System.Guid,passed by reference,that represents the IID of the CLSID interface of the class whose type info is requested.
  """
  pass
 def GetTypeInfoType(self,index,pTKind):
  """
