class Int64Animation(Int64AnimationBase,ISealable,IAnimatable,IResource):
 ...
 def Clone(self):
  ...
  Clone(self: Int64Animation) -> Int64Animation
   ...
   Creates a modifiable clone of this 
    System.Windows.Media.Animation.Int64Animation,making deep copies of this 
 ...
 def GetCurrentValueCore(self,*args):
  ...
  GetCurrentValueCore(self: Int64Animation,defaultOriginValue: Int64,defaultDestinationValue: Int64,animationClock: AnimationClock) -> Int64
   ...
   Calculates the value this animation believes should be the current value for 
    the property.
  
  
   defaultOriginValue: This value is the suggested origin value provided to the animation to be used 
    if the animation does not have its own concept of a start value. If this 
    animation is the first in a composition chain this value will be the snapshot 
    value if one is available or the base property value if it is not; otherwise 
    this value will be the value returned by the previous animation in the chain 
    with an animationClock that is not Stopped.
  
   defaultDestinationValue: This value is the suggested destination value provided to the animation to be 
    used if the animation does not have its own concept of an end value. This value 
    will be the base value if the animation is in the first composition layer of 
    animations on a property; otherwise this value will be the output value from 
    the previous composition layer of animations for the property.
  
   animationClock: This is the animationClock which can generate the CurrentTime or 
    CurrentProgress value to be used by the animation to generate its output value.
